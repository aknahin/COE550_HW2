...
def on_message_received(status, msg, packet):
	...
	if led == '1023' and packet["topic"] == "dim":
		print packet['payload']
		dim = packet['payload']
		analogWrite(0, int(packet['payload']))
	else:
		analogWrite(0, 'LOW')
	if packet["topic"] == "led":
		print dim
...
def main():
	...
	mqttclient.connect("192.168.25.109", "led", "1234")
	while not mqttclient.state()["connected"]:
		pass

	mqttclient.subscribe('led')
	mqttclient.subscribe('dim')
